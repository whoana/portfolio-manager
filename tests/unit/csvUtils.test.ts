...
describe("parseCsvToHoldings", () => {
  ...
  it("쉼표 포함 숫자 처리 (12,500 → 12500)", () => {
    const csv = '배당,TIGER,458730,100,"12,500"';
    const result = parseCsvToHoldings(csv);
  ...
  it("수량 0 이하는 에러", () => {
    const csv = "배당,TIGER,458730,0,12500";
    const result = parseCsvToHoldings(csv);
  ...
  it("평단가 음수는 에러", () => {
    const csv = "배당,TIGER,458730,100,-1000";
    const result = parseCsvToHoldings(csv);
  ...
  it("큰따옴표로 감싼 필드 처리", () => {
    const csv = '"배당","TIGER, 미국배당","458730","100","12500"';
    const result = parseCsvToHoldings(csv);
...
describe("exportHoldingsToCsv", () => {
  it("보유내역을 CSV로 변환", () => {
    const items: HoldingItem[] = [
      { id: "h1", category: "배당", name: "TIGER", code: "458730", quantity: 100, avgPrice: 12500 },
      { id: "h2", category: "성장", name: "KODEX", code: "379800", quantity: 50, avgPrice: 18000 },
    ];
    const csv = exportHoldingsToCsv(items);

    expect(csv).toContain("\uFEFF");
    expect(csv).toContain("구분,종목,종목코드,수량,평단가");
    expect(csv).toContain("배당,TIGER,458730,100,12500");
    expect(csv).toContain("성장,KODEX,379800,50,18000");
  });

  it("종목명에 쉼표 포함 시 큰따옴표 감싸기", () => {
    const items: HoldingItem[] = [
      { id: "h1", category: "배당", name: "TIGER, 미국배당", code: "458730", quantity: 100, avgPrice: 12500 },
    ];
    const csv = exportHoldingsToCsv(items);
  ...
  it("roundtrip: export → import 결과 일치", () => {
    ...
    expect(parsed.items[0].category).toBe("배당");
